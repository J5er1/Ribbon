...
// The fire's drawing (§4.1), shared by the app and its widgets (S24). It
// lives here rather than beside CampfireView because the home screen and
// the lock screen draw the same fire the room does — one painter, so the
// widget's fire can never become a different object from the room's. The
// reasoning about how it draws is in CampfireView.swift; what it needs is
// only the palette and the core's fire types, which is what makes it
// shareable.
