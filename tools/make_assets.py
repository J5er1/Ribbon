...
"""
    ...
    # The notification icon (S19). Android draws a small icon as a mask -
    # it takes the alpha and throws the colour away - so this is the same
    # shape as the monochrome slot and for the same reason: the knockout
    # cannot be a ground-coloured stroke where there is no ground.
    #
    # A 24-unit canvas rather than 108, because that is the size a status
    # bar actually draws, and the mark is inset by 2 units on each side
    # into the 20-unit content area Android's own icons keep. Scaling the
    # 108-unit launcher icon down instead would land the mark at about a
    # third of the canvas, adrift in its own safe circle - which is right
    # for a launcher shape that may be masked to a circle and wrong for a
    # status bar, where nothing is masked and the only thing that matters
    # is being legible at 24 px on a strip of glass.
...
<vector xmlns:android="http://schemas.android.com/apk/res/android"
    ...
    <group
        ...
        android:scaleX="{20.0 / 64.0:.6g}"
        android:scaleY="{20.0 / 64.0:.6g}">
        <path
            android:pathData="{path_d(BACK)}"
            android:fillColor="#FFFFFF" />
        <path
...
"""
    with open(os.path.join(drawable_dir, "ic_notification.xml"), "w") as f:
        f.write(notification)
    print("wrote", os.path.join(drawable_dir, "ic_notification.xml"))
